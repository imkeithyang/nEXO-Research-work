void AnalyzeTreegamma()
{
   ...
   fpx=fopen("/Users/Tiger/nexo_py/txt/gamma_x_pos.txt", "ab+");
   fpy=fopen("/Users/Tiger/nexo_py/txt/gamma_y_pos.txt", "ab+");
   fpz=fopen("/Users/Tiger/nexo_py/txt/gamma_z_pos.txt", "ab+");
   fpE=fopen("/Users/Tiger/nexo_py/txt/gamma_EnergyDeposit.txt", "ab+");
   if(fpx == NULL) exit(-1);
   TFile *f = new TFile("/Users/Tiger/nexo_py/Data/gamma/gamma_seed_1.root");
   if (f == 0) {
   ...
   for (Long64_t i=0;i<numberevents;i++){
     tree -> GetEntry(i);
     if (branchTot->GetValue(i, 0) == 0) continue;
      for(UInt_t j=0;j< 150;j++){
